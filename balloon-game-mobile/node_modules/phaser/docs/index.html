...
<h2>What's new in Phaser 2.4.8</h2><div align="center"><img src="http://phaser.io/images/github/news.jpg"></div>

<blockquote>
<p>19th May 2016</p>
</blockquote>
<p>The release of Phaser 2.4.8 continues our commitment to rapid turn-around of bug fixes and enhancements. Scan through the <a href="#change-log">Change Log</a> for comprehensive details about what is new and updated in this version.</p>
<p>As always, my thanks to all of you who contributed towards this release. Either by opening an issue on GitHub, providing a fix, or just giving encouragement to the dev team.</p>
<p>We are now splitting our time between development of <a href="#lazer">Lazer</a>, and Phaser 2.5. We decided, after much discussion with the community, that we owed it to Phaser to go out on a high, so 2.5 will be the version in which we do just that. After this Phaser will enter the LTS (long-term support) stage of its life.</p>
<p>This is when we impose a feature freeze, locking the API down and responding only to bugs. This is a necessary step to allow us to focus on Lazer while still ensuring Phaser is given the support it deserves. Don't worry, we aren't going to abandon you! We've got your backs.</p>
...
<li>Download as <a href="https://github.com/photonstorm/phaser/archive/v2.4.8.zip">zip</a> or <a href="https://github.com/photonstorm/phaser/archive/v2.4.8.tar.gz">tar.gz</a></li>
<li>Download just the build files: <a href="https://github.com/photonstorm/phaser/releases/download/v2.4.8/phaser.js">phaser.js</a> and <a href="https://github.com/photonstorm/phaser/releases/download/v2.4.8/phaser.min.js">phaser.min.js</a></li>
...
<p><code>&lt;script src=&quot;//cdn.jsdelivr.net/phaser/2.4.8/phaser.js&quot;&gt;&lt;/script&gt;</code></p>
<p>or the minified version:</p>
<p><code>&lt;script src=&quot;//cdn.jsdelivr.net/phaser/2.4.8/phaser.min.js&quot;&gt;&lt;/script&gt;</code></p>
...
<p><code>&lt;script src=&quot;https://cdnjs.cloudflare.com/ajax/libs/phaser/2.4.8/phaser.js&quot;&gt;&lt;/script&gt;</code></p>
...
<h2>Change Log</h2><h2>Version 2.4.8 - &quot;Watch Hill&quot; - 19th May 2016</h2><h3>New Features</h3><ul>
<li>BitmapData.copy, and by extension any method that uses it, including BitmapData.draw, drawGroup and drawFull, now all support drawing RenderTexture objects. These can either be passed directly, or be the textures of Sprites, such as from a call to generateTexture.</li>
<li>Arcade Physics has had a new <code>world</code> argument added to the following functions: <code>distanceBetween</code>, <code>distanceToXY</code>, <code>distanceToPointer</code>, <code>angleBetween</code>, <code>angleToXY</code> and <code>angleToPointer</code>. The argument (which is false by default), when enabled will calculate the angles or distances based on the Game Objects <code>world</code> property, instead of its <code>x</code> and <code>y</code> properties. This allows it to work for objects that are placed in offset Groups, or are children of other display objects (thanks @Skeptron for the thread #2463)</li>
<li>Arcade Physics Body has a new property <code>worldBounce</code>. This controls the elasticity of the Body specifically when colliding with the World bounds. By default this property is <code>null</code>, in which case Body.bounce is used instead. Set this property to a Phaser.Point object in order to enable a World bounds specific bounce value (thanks @VitaZheltyakov #2465)</li>
</ul>
<h3>Updates</h3><ul>
<li>TypeScript definitions fixes and updates (thanks @osev7 @staff0rd @galen-manuel)</li>
<li>Docs typo fixes (thanks @dedoubleyou1 @mortonfox @zeterain)</li>
<li>You can now access the intensity of the Camera shake effect via the getter / setter <code>Camera.shakeIntensity</code>. Useful if you wish to tween the intensity while running. (thanks @drhayes #2443)</li>
<li>The Arcade Physics overlap method would return false if two bodies were overlapping but neither had any velocity (i.e. they were embedded into each other)</li>
<li>PIXI.defaultRenderer is now set to <code>null</code> in Game.destroy, allowing it to be reset if a new Game instance is created on the same page (thanks @xtforgame ##2474)</li>
<li>BitmapData.drawGroupProxy is now capable of iterating through Sprites that have children, and also now uses the world positions for drawing instead. This change updates the functionality of BitmapData.drawGroup.</li>
<li>Text.setStyle has a new argument <code>update</code> which will optionally automatically call <code>updateText</code> after setting the new style (thanks @staff0rd  #2478)</li>
</ul>
<h3>Bug Fixes</h3><ul>
<li>Fixed an issue in the Arcade Physics overlap method where it would only detect overlaps up to the max bias threshold and no further (thanks @rgk #2441)</li>
<li>InputHandler.checkPointerDown and checkPointerOver will now test the worldTransform scale property of a Sprite. If zero it will fast return, where-as before it would incorrectly report an up event (thanks @jaapaurelio #2466)</li>
<li>Fixed a bug in Arcade Physics Body.preUpdate which would incorrectly apply the position of an offset Body (one which has had Body.setSize used on it) when combined with a Sprite with a non-zero anchor (thanks @SBCGames #2470)</li>
<li>If you set Game.renderType to <code>Phaser.HEADLESS</code> it will no longer render the output to the canvas. The canvas is still created (although not added to the DOM), as it's required internally, but no rendering now takes place on it (thanks @ForgeableSum #2464)</li>
<li>Sounds played using the Audio tag, that were paused and then resumed again (either directly in code, or via a game pause event) would not resume from the point at which they paused (thanks @rroylance #2473)</li>
<li>Sounds played using the Audio tag, set to loop, would get caught in an endless pause-play loop cycle (thanks @rroylance #2473)</li>
</ul>
...
<ul>
<li>Sprites that had a tint on them, that then had their frame changed via either <code>Sprite.frame</code> or <code>Sprite.frameName</code> wouldn't re-tint the new frame, and would become stuck on the old frame in Canvas mode (thaks @spayton #2453)</li>
</ul>
